...
_log_file="/opt/haproxy_cfg_backup/switch.log"
_backend_server=$1
_oper=$2
...
/bin/cp $_cfg_file /opt/haproxy_cfg_backup/haproxy.cfg.$_time
case $_oper in
off)
...
on)
    ...
    ;;
*)
    echo "haproxy-backend-switch <server_name> <on>	Switch On"
    echo "haproxy-backend-switch <server_name> <off>	Switch Off"
    echo 
